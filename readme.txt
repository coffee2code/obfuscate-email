...
Any textual occurrence of an e-mail address in a post body, excerpt, or comment (and when admin and user emails are retrieved for display) will be obfuscated (see Filters section for `c2c_obfuscate_email_filters` for complete list of filters that are processed).

"Obfuscation" simply means that techniques are employed to modify e-mail strings that may appear in posts and pages in such a way that bots scraping your site are unable to identify those e-mails; however, the e-mails should still look right and work right for visitors, as much as possible.

The plugin allows for use of one or more (or all!) of three techniques for email protection.  While techniques abound for e-mail obfuscation, the three techniques included empirically provide you with the best balance of e-mail protection with minimal impact on visitors.  You can decide on a technique by technique basis which ones you'd like to employ as some have potential drawbacks.  The plugin's settings page allows you select which techniques to use.
...
*How does it work?*  The `@` and `.` characters are replaced with alternative strings, such as `AT` and `DOT`, respectively.  The exact replacements are configurable in the plugin's settings page.  By default, if you don't specify custom replacements, the plugin will use entity substitution (`@` becomes `&#064;` and `.` becomes `&#046;`).
*Uses CSS?* No.
*Uses JavasScript?* No.
...
*How effective is this?* In the aforementioned experiment, almost no spam emails were received when using just this technique.  However, this technique
*Examples*
...
Links: [Plugin Homepage](http://coffee2code.com/wp-plugins/obfuscate-email/) | [Author Homepage](http://coffee2code.com)
...
No.  This makes this plugin's implementation of obfuscation more compatible and usable by more visitors.  This may leave out techniques that some could argue are aggressively protective, but I feel the included techniques are just as effective.
